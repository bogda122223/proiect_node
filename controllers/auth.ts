...
const deleteUser = async (req: Express.Request, res: Express.Response) => {
    ...
    if (!email || !name || !password) {
      return res.status(401).send({ message: "Required name, email and password" });
    }
    ...
    try {
      ...
      res.status(500).send({ message: "An error occurred while deleting the user", error });
    }
  };
  ...
  const hashedPassword = await bcrypt.hash(password, 12);
    const user = await User.findOneAndUpdate({email}, {name: name, authentication: {
        password: hashedPassword,
      }}, {new: true});
    if (!user) {
        res.status(401).send({ message: "User with same email doesn't exists." });
      } else
      res.status(200).send({ message: "User updated successfully" });
  };

export { registerUser, loginUser, getAllUsers, logout, deleteUser, updateUser};